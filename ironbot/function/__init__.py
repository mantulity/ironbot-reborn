...
import argparse
import wget
from ironbot import bot as borg
import lottie
from ironbot.utils import load_module
...
import asyncio
from ironbot.function.FastTelethon import download_file
import json
import math
import os
import re
...
import os
import aiohttp
from ironbot.function.FastTelethon import upload_file


sedpath = Config.TMP_DOWNLOAD_DIRECTORY
from ironbot import logging
...
async def get_all_modules(event, borg, channel_id):
    ...
    for sed in a_plugins:
        try:
            downloaded_file_name = await borg.download_media(sed, "ironbot/modules/")
...
async def apk_dl(app_name, path, event):
    ...
    for link in result[:1]:
        ...
        for link in result:
            ...
            with open(f"{path}/{name}@Ironbot.apk", 'wb') as f:
                f.write(r.content)
    await event.edit('`Apk, Downloaded. Let me Upload It here.`')
    final_path = f'{path}/{name}@Ironbot.apk'
...
Music Downloaded And Uploaded By Ironbot

Get Your Iron From @Ironbot-reborn"""
...
async def fetch_audio(event, ws):
    ...
    if warner_stark.video:
        ...
        stark_cmd = f"ffmpeg -i {warner_bros} -map 0:a iron.mp3"
        stdout, stderr = (await runcmd(stark_cmd))[:2]
        final_warner = "iron.mp3"
