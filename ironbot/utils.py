...
from ironbot import CMD_LIST, LOAD_PLUG, SUDO_LIST, bot, client2, client3, CMD_HELP
from ironbot.Configs import Config
from ironbot.wraptools import (
    am_i_admin,
    ignore_bot,
    ignore_fwd,
    ignore_grp,
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        import ironbot.modules
        import ironbot.utils

        path = Path(f"ironbot/modules/{shortname}.py")
        name = "ironbot.modules.{}".format(shortname)
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = ironbot.utils
        sys.modules["iron.util"] = ironbot.utils
        sys.modules["userbot.utils"] = ironbot.utils
        sys.modules["userbot.plugins"] = ironbot.modules
        sys.modules["plugins"] = ironbot.modules
        sys.modules["userbot"] = ironbot
        mod.admin_cmd = iron_on_cmd
        mod.sudo_cmd = sudo_cmd
        mod.iron_on_cmd = iron_on_cmd
        ...
        sys.modules["ironbot.events"] = ironbot.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["ironbot.modules." + shortname] = mod
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"ironbot.modules.{shortname}"
...
def iron_on_cmd(pattern=None, **args):
    args["func"] = lambda e: e.via_bot_id is None
...
""" Userbot module for managing events.
 One of the main components of the ironbot. """
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            ftext += "--------BEGIN IRONBOT-REBORN TRACEBACK LOG--------"
            ftext += "\nDate: " + date
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"ironbot/modules/assistant/{shortname}.py")
        name = "ironbot.modules.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        spec.loader.exec_module(mod)
        sys.modules["ironbot.modules.assistant" + shortname] = mod
