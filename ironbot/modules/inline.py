...
from ironbot.function import _ytdl, fetch_json, _deezer_dl, all_pro_s
from urllib.parse import quote
import requests
from telethon import Button, custom, events, functions
from youtubesearchpython import VideosSearch
from ironbot import ALIVE_NAME, CMD_HELP, CMD_LIST, client2 as client1, client3 as client2, bot as client3
from ironbot.modules import inlinestats
...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "IRON"
...
async def inline_handler(event):
    ...
    result = None
    query = event.text
    if event.query.user_id in o and query.startswith("IRON"):
        rev_text = query[::-1]
        ...
        result = builder.article(
            title="Stats",
            text=f"**Status : {DEFAULTUSER} ** \nNote --> Hanya owner ya gan yang bisa klik stats \n(C) @IRONBOTs",
            buttons=[
                [custom.Button.inline("Show Stats ?", data="terminator")],
                [Button.url("Repo 🇮🇩", "https://github.com/mabotsss/ironbot-reborn")],
                [Button.url("Join Channel ❤️", "t.me/freedom_reborn")],
            ],
        )
        await event.answer([result])
        ...
        result = builder.photo(
            ...
            text=query,
            buttons=[
                [custom.Button.inline("Mau Spam", data="dontspamnigga")],
                [custom.Button.inline("Mau Chat", data="askme")],
            ],
        )
        await event.answer([result])
...
async def on_plug_in_callback_query_handler(event):
    ...
    if event.query.user_id in o:
        ...
        reply_popp_up_alert = "Makanya pake Ironbot biar ga kepo wkwkwk.."
...
async def on_plug_in_callback_query_handler(event):
    ...
    if event.query.user_id in o:  # pylint:disable=E0602
        ...
        reply_pop_up_alert = "Makanya pake Ironbot biar ga kepo wkwkwk.."
...
async def on_plug_in_callback_query_handler(event):
    ...
    if not event.query.user_id in o:
        sedok = "Mau ngapaingan ? zzz install Ironbot sendiri lah.."
    ...
    if len(reply_pop_up_alert) >= 4096:
        ...
        await event.edit(
            ...
            link_preview=False,
            buttons=[[custom.Button.inline("<", data=f"backme_{page_number}")]],
        )
    else:
        await event.edit(
...
async def rip(event):
    ...
    if event.query.user_id in o:
        ...
        txt = "Gabisa ngintip dia gan , karena dia penciptaku wkwk"
...
async def rip(event):
    ...
    if event.query.user_id not in o:
        text = f"Mau ngapaingan ? zzz install Ironbot sendiri lah.."
        await event.answer(text, alert=True)
        return
    is_it = True
...
async def rip(event):
    o = await all_pro_s(Config, client1, client2, client3)
    if event.query.user_id in o:
        sedok = "Gaperlu pake ini gan."
        await event.answer(sedok, cache_time=0, alert=True)
        return
    await event.get_chat()
    him_id = event.query.user_id
    text1 = "`Yaa salah sendiri mau spam jadinya diblokir sama Ironbot kan wkwkwk.`"
...
async def sed(event):
    ...
    sed = f"""IRON Userbot Modules Are Listed Here !\n
...
async def rip(event):
    ...
    await event.get_chat()
    him_id = event.query.user_id
    await event.edit("Ok. Tunggu sampai guruku bales yaa..")
...
async def rip(event):
    ...
    await event.get_chat()
    him_id = event.query.user_id
    await event.edit("Ok, Tunggu sampai guruku bales ya..")
...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if event.query.user_id not in o:
        resultm = builder.article(
            title="Not Allowded",
            text=f"Ente gabisa make bot ini gan. \Deploy dulu kalo mau, Repo Link [Here](https://github.com/mabotsss/ironbot-reborn)",
        )
