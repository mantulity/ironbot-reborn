...
import ironbot.modules.sql_helper.pmpermit_sql as pmpermit_sql
from ironbot import ALIVE_NAME, CUSTOM_PMPERMIT
from ironbot.Configs import Config
from ironbot.function import is_nsfw
...
USER_BOT_WARN_ZERO = "Dibilangin jangan spam malah ngespam ... yaaa ke Block lah.."
...
if PM_ON_OFF != "DISABLE":
    ...
    @borg.on(iron_on_cmd(pattern="(a|approve|allow)$"))
    async def approve(event):
        ...
        if event.is_private:
            ...
            if not pmpermit_sql.is_approved(event.chat_id):
                ...
                pmpermit_sql.approve(event.chat_id, "Approved")
                await event.edit(
                    "Diizinkan untuk pm [{}](tg://user?id={})".format(firstname, event.chat_id)
                )
                await asyncio.sleep(3)
                await event.delete()
            elif pmpermit_sql.is_approved(event.chat_id):
                sed = await event.edit('`User ini udah diizinkan bos...`')
                await asyncio.sleep(3)
                await sed.delete()
        elif event.is_group:
            reply_s = await event.get_reply_message()
            ...
            if not pmpermit_sql.is_approved(reply_s.sender_id):
                ...
                await event.edit(
                        "Diizinkan untuk pm [{}](tg://user?id={})".format(firstname, reply_s.sender_id)
                    )
                await asyncio.sleep(3)
                await event.delete()
            elif pmpermit_sql.is_approved(reply_s.sender_id):
                await event.edit('`User ini udah diizinkan bos...`')
                await event.delete()

    @borg.on(iron_on_cmd(pattern="(da|disapprove|disallow)$"))
    ...
    @borg.on(iron_on_cmd(pattern="listapproved$"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
    ...
    async def do_pm_permit_action(chat_ids, event):
        if Config.NSFW_FILTER_PM:
            if event.media:
                ...
                if hehe is True:
                    await event.client.send_message(chat_ids, "`How Dare You Send Nsfw In My Masters Pm, You Have Been Blocked By IRONBot !`")
...
CMD_HELP.update(
    {
        "pmpermit": "**pmpermission**\
\n\n**Syntax : **`.a|approve|allow)`\
\n**Usage :** Allow PM\
\n\n**Syntax : **`.da|disapprove|disallow`\
\n**Usage :** Dissallow PM\
\n\n**Syntax : **`.block`\
\n**Usage :** Block PM\
\n\n**Syntax : **`.listapproved`\
\n**Usage :** list user allowed"
    }
)
