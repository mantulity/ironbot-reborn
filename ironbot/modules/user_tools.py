...
from ironbot.modules.sql_helper.broadcast_sql import (
    add_chnnl_in_db,
    already_added,
    get_all_chnnl,
    rm_channel,
    ...
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if reason:
            await borg.send_message(
                event.chat_id,
                f"__Lagi afk ❗ karena__ `{reason}`",
            )
        else:
            await borg.send_message(event.chat_id, f"`Afk dulu`❗.")
...
@iron.on(events.NewMessage(outgoing=True))  # pylint:disable=E0602
    ...
    if ".afk" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        ...
        try:
            ...
            await borg.send_message(  # pylint:disable=E0602
                ...
                + "Please Seek Support in @IRONBOT\n\n `{}`".format(str(e)),
...
async def on_afk(event):
    ...
    if "afk" in current_message_text:
        # ironbot's should not reply to other ironbot's
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        if afk_time:  # pylint:disable=E0602
            ...
            if days == 1:
                ...
                f"`{int(hours)}h{int(minutes)}m` **yang lalu**"
            elif minutes > 0:
                f"`{int(minutes)}m{int(seconds)}s` **yang lalu**"
            else:
                f"`{int(seconds)}s` **yang lalu**"
        msg = None
        message_to_reply = (f"__Sabar lagi afk__❗\n__selama __ `{total_afk_time}`\n__karena__ `{reason}`" if reason else f"__Sabar lagi afk__❗\n__selama :__ `{total_afk_time}`")
...
@iron.on(iron_on_cmd(pattern="brm ?(.*)"))
@iron.on(sudo_cmd(pattern="brm ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_chnnl = event.pattern_match.group(1)
...
@iron.on(iron_on_cmd(pattern="broadcast$"))
@iron.on(sudo_cmd(pattern="broadcast$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@iron.on(iron_on_cmd(pattern="bforward$"))
@iron.on(sudo_cmd(pattern="bforward$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    all_chnnl = get_all_chnnl()
...
@iron.on(iron_on_cmd(pattern="bstat"))
@iron.on(sudo_cmd(pattern="bstat", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(iron_on_cmd(pattern="cr (.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@iron.on(iron_on_cmd(pattern="clone ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    await event.delete()
    await borg.send_message(
        event.chat_id, "**MANTAP SEKARANG UDAH SAMA**", reply_to=reply_message
    )
