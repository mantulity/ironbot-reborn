...
from ironbot.function import get_all_admin_chats, is_admin
from ironbot.modules.sql_helper.night_mode_sql import add_nightmode, rmnightmode, get_all_chat_id, is_nightmode_indb
...
@iron.on(iron_on_cmd(pattern="scgrp$"))
...
@iron.on(iron_on_cmd(pattern="rsgrp$"))
    ...
    for warner in ws_chats:
        try:
            ...
            if Config.CLEAN_GROUPS:
                async for user in iron.iter_participants(int(warner.chat_id)):
                    if user.deleted:
                        await iron.edit_permissions(int(warner.chat_id), user.id, view_messages=False)
